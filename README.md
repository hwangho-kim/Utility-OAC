...
저온/고온 냉동기 시스템별로 주요 지표(메인 차압, 메인 차압 개도율, 층별 서브 차압 개도율(F3/F4)) 예측 모델(RandomForestRegressor) 학습. 학습 시 냉동기 메인 냉수 온도차(delta_T)와 파생된 가동 대수를 주요 특징으로 사용.
...
최적화된 냉동기 운전 조건에 기반하여 메인 냉수 온도차(delta_T)를 추정하고, 이를 사용하여 냉동기 시스템 지표 예측.
...
외부 키: OAC 유닛의 일반 ID 문자열 (예: 'oac_0', 'oac_1', ...). 이 키는 HVACSystemFacade 내부에서 oac_predictions_all_units의 키(예: F03_OAC_00_nonAI)와 매칭됩니다.
...
목적: OAC의 전체 시스템 내 순차 인덱스(0~13)를 입력받아, 층(F3/F4), 해당 층 내 인덱스(00~06), AI 모드 유무(AI/nonAI) 정보를 포함하는 표준화된 이름 접두사(예: "F03_OAC_00_nonAI_")를 생성합니다.
...
목적: 모델 학습 및 시스템 테스트를 위한 상세 시계열 더미 데이터를 지정된 주기(기본 10분)로 생성합니다. 사용자가 명시한 피처 목록에 해당하는 데이터만 생성하며, 파생 피처(예: 엔탈피, 냉동기 가동 대수 등)는 이 함수에서 직접 생성하여 데이터프레임에 포함시키지 않습니다. OAC 피처 이름은 get_oac_name_prefix를 사용하여 표준화된 명명 규칙을 따릅니다.

파라미터: num_days, freq_minutes, num_oac, num_low_ch, num_high_ch, num_ct.

출력: pandas.DataFrame. 명시된 스키마에 따른 컬럼들만 포함.

OAC: 각 OAC 유닛(이름에 층/AI모드 포함)에 대해 _set_point_temp_c, _coil_open_rate, _후단온도_c, _후단습도_percent (가습 코일 제외), _토출온도_c, _토출노점온도_c, _토출압력_pa.
...
내부 계산용 임시 변수: _internal_total_low_temp_chiller_load_watts_series, _internal_total_high_temp_chiller_load_watts_series는 함수 내부에서 OAC 부하를 시뮬레이션하고 개별 냉동기 전력값을 생성하는 데 사용되지만, 최종 반환되는 DataFrame에는 포함되지 않습니다.

3.4. create_dummy_csv_second_data 함수

목적: CSV 파일 처리 기능 테스트를 위한 초당 더미 CSV 파일을 생성합니다. generate_dummy_data와 유사하게 명시된 피처들만 포함합니다.
...
목적: 입력 데이터프레임에서 각 냉각탑의 _supply_수조레벨_percent를 기반으로 가동 상태(_is_active)를 판단하고, 총 가동 냉각탑 수(derived_total_active_cooling_towers)를 계산하여 컬럼으로 추가합니다.
...
목적: 시스템 내 모든 OAC 유닛 각각에 대해, 각 코일의 개도율을 예측하는 독립적인 모델들을 학습, 예측, 저장 및 로드합니다. 각 OAC 유닛은 개별적인 목표 온도 설정값을 입력받아 사용하며, 가습 코일 후의 상태는 OAC의 최종 토출 상태입니다. 엔탈피 관련 계산은 kJ/kg 단위를 사용합니다.
...
train_coil_models(df_train_data_full): OAC 유닛별, 코일별 모델 학습. 입력 df_train_data_full은 명시된 피처만 포함. 엔탈피 등 필요한 파생 피처는 메소드 내부에서 동적으로 계산.

predict_coil_open_rates(outdoor_temp_c, outdoor_rh_percent, oac_set_points_by_unit, modified_oac_coil_states_all_units=None): OAC 유닛별 예측 수행. oac_set_points_by_unit으로 유닛별 SP 입력받음.
...
train_pressure_valve_models(df_train_data): 모델 학습. 입력 df_train_data는 _active_count가 파생되어 포함된 상태. _main_delta_t_c는 이 메소드 내에서 main_supply/return_온도_c로부터 동적 계산.

학습 특징: 동적으로 계산된 _main_delta_t_c와 파생된 _active_count.
...
목적: 냉동기 최적 운전 결정 (평균 부하율 50% 내외, 최대 부하율 70% 제약).
...
OACModel 호출 시 oac_set_points_by_unit 전달하여 OAC 예측.

oac_meta_data_by_unit의 AI 모드 정보를 참조하여 저온/고온 냉동기 부하 분리 계산.
...
전역 설정 (DEBUG_MODE, TRAIN_NEW_MODELS, MODEL_SAVE_DIR, 임계값들).

데이터 소스 선택 및 load_and_preprocess_data 호출 (내부적으로 active_count 등 파생).

TRAIN_NEW_MODELS 플래그에 따라 모델 신규 학습/저장 또는 기존 모델 로드.

학습된 모델 정보(일부) 출력.

HVACSystemFacade 객체 생성.

하절기 시나리오 실행: OAC 유닛별 SP 및 메타데이터(oac_meta_data_by_unit_summer) 생성하여 Facade에 전달.

결과 출력 (OAC 유닛별 상세 정보, 개도율 요약 테이블, 냉동기/냉각탑 최적화 결과).
